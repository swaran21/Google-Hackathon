...
export default function DriverPage() {
  ...
  const myAmbulanceId = user?.assignedAmbulance?._id || user?.assignedAmbulance || null;
  ...
  const startSimulation = () => {
    ...
    simRef.current = setInterval(async () => {
      try {
        const res = await api.post('/driver/simulate-move', { ambulanceId: myAmbulanceId });
        ...
        if (arrived) {
          ...
          if (arrivalLabel === 'patient') {
            showToast('Arrival Confirmed: Unit at patient scene. Ready for Phase 2.', 'success');
          } else if (arrivalLabel === 'hospital') {
            showToast('Mission Complete: Patient delivered to hospital.', 'success');
            setSimPhase(null);
          }
        }
  ...
  useSocket(socket, 'ambulance:location-update', (data) => {
    ...
    setDriverData(prev => prev ? {
      ...prev,
      ambulance: { ...prev.ambulance, location: data.location, status: data.status },
    } : prev);
  });
  ...
  const coords = useMemo(() => {
    const hasCoords = (item) => Array.isArray(item?.location?.coordinates) && item.location.coordinates.length === 2;
    
    return {
      amb: hasCoords(amb) ? [amb.location.coordinates[1], amb.location.coordinates[0]] : null,
      em: hasCoords(em) ? [em.location.coordinates[1], em.location.coordinates[0]] : null,
      hosp: hasCoords(em?.assignedHospital) ? [em.assignedHospital.location.coordinates[1], em.assignedHospital.location.coordinates[0]] : null,
    };
  }, [amb, em]);
  ...
  const routeLine = useMemo(() => {
    ...
    if (isPhase2 && coords.hosp) {
      return { positions: [coords.amb, coords.hosp], color: '#3b82f6', label: 'To Hospital' };
    }
    if (coords.em && !isPhase2) {
      return { positions: [coords.amb, coords.em], color: '#ef4444', label: 'To Patient' };
    }
    return null;
  }, [coords, isPhase2]);
  ...
  const phaseInfo = useMemo(() => {
    ...
    if (amb.status === 'dispatched') return { label: 'AWAITING ACCEPT', color: '#facc15', bg: 'rgba(250,204,21,0.1)' };
    if (amb.status === 'en_route') return { label: 'PHASE 1: EN ROUTE TO PATIENT', color: '#ef4444', bg: 'rgba(239,68,68,0.1)' };
    if (amb.status === 'at_scene' && em.assignedHospital) return { label: 'PHASE 2: TRANSPORTING TO HOSPITAL', color: '#3b82f6', bg: 'rgba(59,130,246,0.1)' };
    if (amb.status === 'at_scene') return { label: 'AT SCENE', color: '#a855f7', bg: 'rgba(168,85,247,0.1)' };
    return null;
  }, [amb, em]);
  ...
  if (!myAmbulanceId) {
    return (
      <div className="page-enter" style={{ minHeight: '100vh', padding: '80px 24px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="glass-card" style={{ padding: '40px', borderRadius: '24px', textAlign: 'center', maxWidth: '420px' }}>
          <Navigation size={40} style={{ color: '#ef4444', marginBottom: '16px' }} />
          <h2 style={{ fontWeight: 900, fontSize: '1.25rem', marginBottom: '8px' }}>No Ambulance Assigned</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Your account is not linked to any ambulance unit. Contact your admin to get assigned.</p>
        </div>
      </div>
    );
  }
  ...
  if (loading && !driverData) {
    return (
      <div className="page-enter" style={{ minHeight: '100vh', padding: '80px 24px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Loader2 size={32} style={{ color: '#ef4444', animation: 'spin 1s linear infinite' }} />
      </div>
    );
  }
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '20px', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div className="glass-card" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: '10px', borderRadius: '14px' }}>
              ...
              <span style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-primary)', fontFamily: 'monospace' }}>{amb?.vehicleNumber || '...'}</span>
            </div>
            {phaseInfo && (
              <div style={{ padding: '8px 16px', borderRadius: '14px', background: phaseInfo.bg, border: `1px solid ${phaseInfo.color}30`, display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: phaseInfo.color, boxShadow: `0 0 8px ${phaseInfo.color}`, animation: 'pulse-glow 2s ease-in-out infinite' }} />
                <span style={{ fontSize: '10px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: phaseInfo.color }}>{phaseInfo.label}</span>
              </div>
            )}
            <div className="glass-card" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: '10px', borderRadius: '14px' }}>
              <Activity size={16} style={{ color: '#22c55e' }} />
              <span style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-secondary)' }}>Online</span>
            </div>
          </div>
        </div>

        {/* Full-width map + controls (NO sidebar fleet list) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, minHeight: '500px' }}>
          <div style={{ 
            position: 'relative', 
            flex: 1, 
            borderRadius: '28px', 
            ...
            >
              ...
              {coords.em && !isPhase2 && <Marker position={coords.em} icon={emIcon} />}
              {coords.hosp && <Marker position={coords.hosp} icon={hospIcon} />}
              
              {/* Phase-aware route polyline */}
              {routeLine && <Polyline positions={routeLine.positions} pathOptions={{ color: routeLine.color, weight: 3, dashArray: '10, 10', opacity: 0.6 }} />}
            </MapContainer>
            ...
            {phaseInfo && (
              <div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 1000, background: isDark ? 'rgba(10,10,15,0.85)' : 'rgba(255,255,255,0.9)', backdropFilter: 'blur(20px)', border: `1px solid ${phaseInfo.color}30`, padding: '10px 14px', borderRadius: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Truck size={14} style={{ color: phaseInfo.color }} />
                <span style={{ fontSize: '10px', fontWeight: 800, color: phaseInfo.color, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            ...
            {/* Map legend */}
            <div style={{ position: 'absolute', bottom: '16px', left: '16px', zIndex: 1000, background: isDark ? 'rgba(10,10,15,0.88)' : 'rgba(255,255,255,0.9)', backdropFilter: 'blur(16px)', border: '1px solid var(--border-glass)', padding: '10px 14px', borderRadius: '12px', fontSize: '11px', fontWeight: 700, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
              {coords.amb ? (
                <>
                  <span>🚑 You</span>
                  {coords.em && !isPhase2 && <><span>•</span><span>🆘 Patient</span></>}
                  {coords.hosp && <><span>•</span><span>🏥 Hospital</span></>}
                </>
              ) : (
            ...
            {amb ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: '1.35rem', fontWeight: 900 }}>{amb.vehicleNumber}</h3>
                    <p style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{amb.driverName} • {amb.equipmentLevel?.replace('_', ' ')}</p>
                  </div>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px' }}>
                  ...
                  {amb.status === 'en_route' && (<>
                    {actionBtn(simulating ? stopSimulation : startSimulation, simulating ? 'rgba(234,179,8,0.12)' : '#2563eb', simulating ? '#eab308' : '#fff', simulating ? <Pause size={16} /> : <Play size={16} />, simulating ? 'PAUSE GPS' : 'NAVIGATE TO PATIENT', simulating ? { border: '1px solid rgba(234,179,8,0.3)' } : {})}
                    {actionBtn(() => handleAction('update-status', { ambulanceId: amb._id, status: 'at_scene' }, 'Unit At Scene — Patient Picked Up'), 'var(--bg-glass)', 'var(--text-primary)', <MapPin size={16} />, 'MARK PATIENT PICKED UP', { border: '1px solid var(--border-glass)' })}
                  </>)}
                  {amb.status === 'at_scene' && em?.assignedHospital && (<>
                    {actionBtn(simulating ? stopSimulation : startSimulation, simulating ? 'rgba(234,179,8,0.12)' : '#3b82f6', simulating ? '#eab308' : '#fff', simulating ? <Pause size={16} /> : <Play size={16} />, simulating ? 'PAUSE GPS' : 'NAVIGATE TO HOSPITAL', simulating ? { border: '1px solid rgba(234,179,8,0.3)' } : {})}
                    {actionBtn(() => handleAction('update-status', { ambulanceId: amb._id, status: 'available' }, 'Emergency Resolved — Patient Delivered'), '#16a34a', '#fff', <Shield size={16} />, 'COMPLETE MISSION')}
                  </>)}
                  {amb.status === 'at_scene' && !em?.assignedHospital &&
                    actionBtn(() => handleAction('update-status', { ambulanceId: amb._id, status: 'available' }, 'Emergency Resolved'), '#16a34a', '#fff', <Shield size={16} />, 'COMPLETE MISSION & RETURN')
                  }
              ...
              <div style={{ height: '72px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', fontSize: '12px', letterSpacing: '0.15em' }}>Loading unit telemetry...</div>
            )}
          </div>
        </div>
      </div>
